...
#Search Arulesvisualisation on Google
...
#Y always has to be a single item, X can have a basket of multiple items
frequentItems = eclat (Groceries, parameter = list(supp = 0.01, minlen= 2, maxlen = 5)) #minlen = 2 means there should be at least 1 X and 1 Y in the basket 
frequentItems = eclat (Groceries, parameter = list(supp = 0.009, minlen= 4, maxlen = 5))
...
inspect(subset2)
subset2a = subset(rules2, subset=lhs %in% c('baking powder','soda') ) #either bakin powder or soda
inspect(subset2a)
...
inspect(subset3)
subset4 = subset(rules2, subset=lhs %in% 'bottled beer' & rhs %in% 'whole milk' ) #either bottled beer or whole milk in LHS
inspect(subset4)
