...
head(women)
model1=lm(weight ~ height, data=women[-c(1,14,15), ])
summary(model1)
ndata = data.frame(height=c(59,65, 64.5, 70))
range(women$height)
predict(model1, newdata = ndata)
?predict
cbind(ndata, predict(model1, newdata = ndata))
coef(model1)
x=59
(y = -87 + 3.45 * x)
plot(model1)
